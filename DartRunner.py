...
class DartRunner(object):
    ...
    def __init__(self, metaDict, clusterList , testRunFile, loadDatabase = True):
        ...
        self.noTestLog = False
        self.noConsoleLog = False
        
        if "noTestLog" in metaDict:
            self.noTestLog = metaDict['noTestLog']
            
        if "noConsoleLog" in metaDict:
            self.noConsoleLog = metaDict['noConsoleLog']    
        ...
        dlog.info('Current Process Id: %s'%currentPid)
        #End of Logger Setup
    ...
    def main(self):
        ...
        if self.installTest:
            ...
            for cluster in self.clusters:
                ...
                self.testReports[testCnt] = ['InstallTest', cluster, 'Skipped', 'Install']
                testCnt = testCnt + 1  
            ...
            for cluster in self.clusters:
                ...
                if 'NAME' in test:
                    testName = test['NAME']
                    testId = test['testId']
                    self.testReports[testId] = [testName, cluster, 'Skipped', 'Install']
                else:
                ...
                self.testReports[testId] = [testName, cluster, 'Running', 'Install']
        ...
        for testSet in self.testSuite:
            ...
            for test in tests:
                ...
                self.testReports[testCnt] = ['', '', 'Skipped' ,testSet]
        ...
        while (self.getTestWaitingCnt() > 0) and ( not self.isAllClustersStale() ) and canProceed:
            ...
            while (self.getFreeClusterCnt() > 0) and (self.getTestWaitingCnt() > 0) and (testExecCount < len(self.allTests)):
                ...
                if 'SETUP' in test:
                    if test['SETUP'] == "True":
                        ...
                        self.testReports[testId] = [test['NAME'], '', 'Setup', 'Setup']
                        continue
                ...
                if 'TEARDOWN' in test:
                    if test['TEARDOWN'] == "True":
                        ...
                        self.testStatus[testId] = 'SKIP'
                        self.testReports[testId] = [test['NAME'], '', 'TearDown', 'TearDown']
                        continue
                ...
                for testSetForTearDown in testSetList:
                    ...
                    if len(tearDownTestList) != 0:
                        ...
                        self.tearDownStatus[testSetForTearDown] = tearDownComplete
                ...
                self.testReports[testId] = [testName, cluster, 'Running' ,testSet]
            ...
            while not queue.empty():
                ...
                testSetName = testReturn['testSet']
                ...
                if testResult:
                    self.testReports[testId] = [testName, cluster, 'Pass' ,testSetName]
                else:
                    self.testReports[testId] = [testName, cluster, 'Fail' ,testSetName]
                ...
                if testTimeOutValues['STARTTIME'] and not testTimeOutValues['ENDTIME']:
                    ...
                    if elapsedTime > timeout:
                        ...
                        if status == 'RUNNING':
                            ...
                            self.testReports[testId] = [testName, cluster, 'Abort' ,testSetName]
        ...
        if self.threshold > 0:
            thresholdStatus = self.verifyTestResults(self.testReports,self.threshold)
        dlog.info('Dart DashBoard URL: %s'%self.dashBoardURL)
        
        try:
            jsonFile = os.path.join(self.logDir, "DartRunner-" + self.timeStamp + '.json')
            with open(jsonFile, 'w') as fileH:
                json.dump(self.testReports, fileH)
        except Exception as e:
            dlog.info('Unable to Dump the TestResults to the Json File!')
            dlog.info(e)
    ...
    def runTest(self, testName, test, cluster, queue, timeout):
        try:
            ...
            if self.noTestLog and not self.noConsoleLog:
                dlog.removeHandler(self.ch)
            darttest = Dart(configFile=clusterConfig, testFile=testDict, dartRunner=True,loadDatabase=self.loadDatabase)
            testResults = darttest.getResults()
    ...
    def verifyTestResults(self, testReports, threshold):
        ...
        failCount = 0
        totalCount = 0
        for testId in testReports:
            testReport = testReports[testId]
            testSet = testReport[3]
            if testSet == 'Install':
                continue
            testResult = testReport[2]
            if testResult == 'Pass':
                passCount = passCount + 1
            if testResult == 'Fail' or testResult == 'Abort' or testResult == 'Skipped' or testResult == 'Running':
                failCount = failCount + 1
        totalCount = passCount + failCount
        
        dlog.info('The number of tests Passed (excludes Installs): %d'%passCount)
        dlog.info('The number of tests Failed (excludes Installs): %d'%failCount)
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument('--noConsoleLog', required=False, default=None, action='store_true', help='Disable Logging on the Console'),
        ...
        metaDict = {'projectName' : args.projectName,
                    ...
                    'noConsoleLog' : args.noConsoleLog,
