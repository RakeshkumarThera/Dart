...
class TestBase(object):
    def __init__(self, cfgJson, testParams={}):
        ...
        if not self.localTest:
            ...
            if "hdfs_namenodes" in self.clusterDict:
                ...
                self.hdfs_namenodes = list()
            if "loaderIps" in self.clusterDict:
                self.loaderIps = self.clusterDict['loaderIps']
            else:
                self.loaderIps = list()
            ...
            if self.cluster.loaderNames != None:
                self.loaderNames = self.cluster.loaderNames

            if self.deployment is not None and self.deployment == "docker":
                self.kubeInstance = Kubernetes(self.cfgJson)
                self.queenExecServicePort, self.configServicePort = self.getQueenExecServicePort()

            
            if self.deployment and self.deployment.lower() == 'docker' and \
                self.clusterType and self.clusterType == 'AWS' and self.actLoc:
                import random
                self.localPort = random.randint(10000,30000)
                dlog.info('Using local Port: %d' % self.localPort)
                ...
                self.tunnel = SSHTunnelForwarder(
                    ...
                    ssh_pkey=self.privateKey,
                    remote_bind_address=(self.queenNames[0], 30002),
                    )
                self.tunnel.start()
    ...
    def getQueenExecServicePort(self):
        # Get the service ports if Aster is up and running, else return defaults
        queenExecServicePort = '30002'
        configServicePort = '30003'
        commandStr = 'kubectl get pods --namespace=cloud-aster'
        stdout, stderr, status = self.kubeInstance.execKubeMaster(commandStr, timeout=30)
        dlog.info(stdout)
        if (not stdout) or ('kubectl: command not found' in stdout):
            dlog.info("No Aster cluster installed on this vms. Returns default values")
            return queenExecServicePort, configServicePort
        else:
            port_cmd = "kubectl get svc queenexec-service -o json jsonpath={.spec.Ports} -n cloud-aster"
            stdout, stderr, status = self.kubeInstance.execKubeMaster(port_cmd, timeout=60)
            if (stdout):
                try:
                    services = json.loads(stdout)
                    ports = services["items"][0]["spec"]["ports"]
                    for item in ports :
                        if item["name"] == "queenexec-p1":
                            queenExecServicePort = item["nodePort"]
                        if item["name"] == "queenexec-p2":
                            configServicePort = item["nodePort"]
                except Exception as e:
                    dlog.info(stdout)
                    dlog.error("Loading the stdout failed with %s" % e)
            return queenExecServicePort, configServicePort
    ...
    def loaderPut(self, localFile, remoteFile, node=0, timeout=300):
        '''
        Sftp a localFile to the Loader location
        '''
        timeoutValue = timeout * self.timeScale
        return self.cluster.loaderPut(localFile, remoteFile, node=node, timeout=timeoutValue)
    ...
    def loaderGet(self, remoteFile, localFile, node=0, timeout=300):
        ...
        '''
        timeoutValue = timeout * self.timeScale
        return self.cluster.loaderGet(remoteFile, localFile, node=node, timeout=timeoutValue)
    ...
    def openLoaderInteract(self, node=0, timeout=60, display=False):
        ...
        '''
        timeoutValue = timeout * self.timeScale
        return self.cluster.openLoaderInteract(node=node, timeout=timeoutValue, display=display)
    ...
    def namenodeExecCommand(self, commandStr, node=0, timeout=60):
        ...
        '''
        timeoutValue = timeout * self.timeScale
        dlog.info('Running on NameNode Command: %s' % commandStr)
        return self.cluster.namenodeExecCommand(commandStr, node=node, timeout=timeoutValue)
    ...
    def excNcliCommand(self, ncliCommandStr, node=0, timeout=120, display=False, cmdShPromt="beehive@"):
        ...
        out = ''.join(out)
        if 'json' in ncliCommandStr:
            matching_start_val = re.search(r'\[', out).start()
            out = out[matching_start_val:]
        else:
            dlog.info("Not a json format string")
        return out, status
    ...
    def copyRemoteConfigFileFromMaster(self, localFile):

        try:
            self.kubeInstance.kubeMasterGet('/var/lib/kubelet/remote-kubeconfig', localFile)
        except IOError as e:
            dlog.info("Failed to copy remote config to local")
            return False
        dlog.info("Succefully copied remote config to local")
        return True
        #set KUBECONFIG variable
        #env_string = "KUBECONFIG=%s" % localFile
	    #os.environ["KUBECONFIG"] = localFile
	    #cmd = 'echo $KUBECONFIG'
	    #returnStatus = self.execCmdLocal(cmd)
        #dlog.info(returnStatus)

        #cmd = 'kubectl get nodes'
        #status, stdout, stderr = self.execCmdLocal(cmd)
	    #dlog.info(stdout)
        #if 'Ready' not in stdout:
        #    dlog.info("Failed to set kubeconfig env variable!")
        #    return False

        #dlog.info("Successfully set env variable!")
        #return True
