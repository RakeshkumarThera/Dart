...
ret = execCmdLocal('docker login sdartifact.td.teradata.com:7002 -u da230151 -p \'CALLORuter92@$\' ')
print(ret) 
...
for line in outlines:
    ...
    if line.strip() != '':
    	image = line.split()[0]
    	print(image)
	tagStr = 'docker tag %s:%s sdartifact.td.teradata.com:7002/asterdata/beehive/drops/qa/%s/%s:%s'%(image,revision,revision,image,revision)
	#print(tagStr)
        ret = execCmdLocal(tagStr)
        #print(ret) 
	pushStr = 'docker push sdartifact.td.teradata.com:7002/asterdata/beehive/drops/qa/%s/%s:%s'%(revision,image,revision)
	print(pushStr)
