...
class DartRunner(object):
    ...
    def __init__(self, metaDict, clusterList , testRunFile, loadDatabase = True):
        ...
        logFile = os.path.join(logDir, "DartRunner-" + timeStamp + '.log')
        ...
        dlog.addHandler(ch)
        dlog.addHandler(fh)
        dlog.info("Logfile is %s" % logFile)
        #End of Logger Setup
    ...
    def main(self):
        ...
        if self.installTest:
            ...
            for cluster in self.clusters:
                ...
                if 'LOCATION' in test:
                    ...
                    dlog.info('The Install Test requires either a LOCATION! Skipping the Install Test!')
                    dlog.info('Cannot use this cluster! Marking the cluster: %s as stale!' % cluster)
                    self.clusterState[cluster] = 'stale'
                    dlog.info(test)
                    continue
        ...
        while (self.getTestWaitingCnt() > 0) and ( not self.isAllClustersStale() ) and canProceed:
            ...
            if mainLoopCnt > 2000:
                dlog.info('Exceeded more than 2000 Main Loops. Exiting the Main Loop!')
                canProceed = False
        ...
        for conn in conns:
            #Wait till all the threads are complete!
            dlog.info('Waiting for all the threads to complete!')
            conn.join()
            dlog.info('All the threads are closed!')
    ...
    def parseUrlToGetBuildNumberAndRevision(self, clusterList, url):
        ...
        self.downloadRpm(clusterList, url)
    ...
    def getQueenIpUserNameAndPassword(self, clusters):
        ...
        for cluster in clusters:
            ...
            self.executeRemoteCmd(clusterDict)
    ...
    def executeRemoteCmd(self, clusterDict):
        try:
            ...
            queenCon.connect()
            stdout, stderr, status = queenCon.execCommand("rm -rf /tmp/rpms/archive")
            stdout, stderr, status = queenCon.execCommand("mkdir -p /tmp/rpms/archive/release")
            for workerIp in clusterDict['workerNodes']:
                workerCon = SshConnect(workerIp, username, password)
                workerCon.connect()
                stdout, stderr, status = workerCon.execCommand("rm -rf /tmp/rpms/archive")
        except:
        ...
        try:
            for cluster in clustersInfo:
                ...
                for localFile in rpmList:
                    ...
                    conn.put(localFile, "/tmp/rpms/archive/release/" + baseFileName)
        except:
    ...
    def insertRunInfo(self, runId, runStartTimeStr):
        ...
        meta = {"run_id": runId, "start_time": runStartTimeStr, "release_name": self.projectName,
                ...
                "tester": self.tester, "run_label": self.runLabel}
        ...
        for cluster in clusterList:
            ...
            if self.clusterState[cluster] == 'free' and not testSetStatus['Failed']:
                if requiredClusterState == 'UP':
                    if self.checkHealthFailCnt[cluster] > 0:
...
if __name__ == "__main__":
    ...
    try:
        ...
        metaDict = {'projectName' : args.projectName, "buildName" : args.buildName,
                    'tester' : args.tester, 'uploadTL' : args.uploadTestLink,
                    'installTest' : args.installFile, 'checkHealthTest' : args.checkHealthFile,
                    'buildNumber' : args.buildNumber, 'branchName' : args.branchName, 'revisionNumber' : args.revisionNumber,
                    'timeScale' : args.timeoutScale, 'testPlan': args.testPlan, 'buildUrl': args.buildUrl,
                    'testTag' : args.testTag,
                    'runLabel': args.runLabel, 'buildLoc' : args.buildLoc}
        dlog.info(metaDict)
